...
def remove_inactive_orders(request):    
    ...
    for od in orders:        
        if od.is_expired():
            ...
            od.delete()
    return HttpResponse(msg+'</ul><a href="/admin/">back to admin</a> <h3><strong>Note:</strong> Instead of manually deleting, this must be run automatically using celery</h3>')
